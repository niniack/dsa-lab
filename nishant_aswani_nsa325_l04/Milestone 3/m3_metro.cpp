...
void List :: remove(string stationName){
  ...
  // if there exists at least one node
    ...
    // if the next value is nullptr, then this element does not exist
    if (walk == nullptr){
      cout << "Sorry, this element does not exist." << endl;
      return;
    }

    // otherwise assume the element exists
    else{
...
void List :: reverse(){
  // edge case where the list does not exist, or there is one element
  // do nothing
  if(listSize<=1){
    return;
  }

  Node *prev = new Node();    // pointer to previous
  Node *walk = new Node();    // pointer to current
  Node *next = new Node();    // pointer to next
  ...
  // note: imagine a list left to right
  // goal is to make it right to left

  // while the main traversal is not pointing to null
  while(walk != nullptr){

    // set the next node to be reversed
    next = walk->next;
    // set the current node to the previous node
    walk->next = prev;
    // make the previous node the one just reversed
    prev = walk;
    // move on to the node on the right;
    walk = next;
  }

  // finally the walk and next pointers point to null
  // only the prev pointer points to the head
  head = prev; 
}
