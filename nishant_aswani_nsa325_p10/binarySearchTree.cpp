...
Node* merge(BST t1, BST t2, int h1, int h2) {
  ...
  if (h1 <= h2){
    newRoot->data = t1._getMaxNode(t1.getRoot())->data;
    t1.deleteNode(t1.getRoot(), newRoot->data);
    newRoot->left = t1.getRoot();
    newRoot->right = t2.getRoot();
  }

  else{
    newRoot->data = t2._getMinNode(t2.getRoot())->data;
    t2.deleteNode(t2.getRoot(), newRoot->data);
    newRoot->left = t1.getRoot();
    newRoot->right = t2.getRoot();
  }
...
Node* BST::_getMaxNode(Node* top){
  Node* walk = top;

  while (walk && walk->right != nullptr){
    walk = walk->right;
  }
...
void BST::deleteNode(Node* top, int data){
  ...
  // Case 2: has two children
    ...
    Node* successor = _getMinNode(walk->right);
